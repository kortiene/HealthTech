...
| **CTRL-32** | **Résilience hors-ligne sans perte de données** (US-2.4, KPI « 100 % des consultations sans perte même en coupure totale ») : l'ordonnance chiffrée est enfilée dans une **file locale chiffrée (SQLCipher)** au lieu d'être perdue quand l'envoi échoue ; **rien en clair** sur l'appareil (ciphertext AES-256-GCM dans une base SQLCipher AES-256, clé scellée Keystore) ; le wipe RAM de la clé de session reste préservé | Technique | [ADR 0006](../adr/0006-offline-storage-and-keys.md), [#21](https://github.com/kortiene/HealthTech/issues/21) | **Partiel** (file `OfflineUploadQueue` + impl. SQLCipher `SqlCipherUploadQueue` + enqueue-on-failure dans `SessionEndService` livrés — `app-patient/lib/src/doctor/{offline_upload_queue,sqlcipher_upload_queue,session_end_service}.dart` ; logique couverte host-only par l'impl. in-memory, liaison SQLCipher réelle = e2e device-backed [#29] ; drain réseau = [#22](https://github.com/kortiene/HealthTech/issues/22)) |
...
| **PREUVE-27** | **Test « consultation validée hors-ligne sans perte »** (échec d'envoi → blob chiffré enfilé, RAM wipée, ciphertext opaque ≠ clair) + **durabilité/illisibilité SQLCipher** | CTRL-32 | [#21](https://github.com/kortiene/HealthTech/issues/21) ; `app-patient/test/doctor/session_end_service_test.dart`, `app-patient/test/e2e/consultation_loop_e2e_test.dart` | **Partiel** (chemin hors-ligne + opacité couverts host-only ; durabilité WAL + illisibilité sans clé = e2e device-backed, suivi [#29]) |
