...
describe("App shell — session helpers re-validated after UX norm update (#28)", () => {
  ...
  it("IDLE_TIMEOUT_MS is 10 min (ADR 0002 — wipe-on-idle, #122)", () => {
  ...
  it("quiet window is 8 min — warning fires at IDLE_TIMEOUT_MS − WARN_BEFORE_MS (#122)", () => {
...
describe(
  ...
  () => {
    ...
    it("IDLE_TIMEOUT_MS is 10 min — session-expiry timeout (wipe-on-idle, #29/#122)", () => {
      expect(IDLE_TIMEOUT_MS).toBe(10 * 60 * 1000);
    });
