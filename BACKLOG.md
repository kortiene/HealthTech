...
- **#29 — Accessibilité & robustesse sur smartphones d'entrée de gamme** · `Should` · `M` · `ux` `tech-debt`
  ...
  *Avancement :* **profil + protocole + source de vérité livrés.** Profil d'appareil de référence opposable ([`docs/ux/low-end-device-profile.md`](./docs/ux/low-end-device-profile.md) — RAM 2–3 Go, 32 Go « quasi saturé » < 500 Mo, API min, densité/écran, réglages d'accessibilité, budgets dérivés, profil réseau `3G-STABLE` #27) et protocole de validation robustesse & accessibilité ([`docs/ux/low-end-validation-protocol.md`](./docs/ux/low-end-validation-protocol.md) — deux parcours, scénario « stockage saturé », scénario « micro-coupure » avec points d'injection, grille d'accessibilité, perf sous contrainte, critères, gabarit de compte-rendu « à produire », éthique/consentement #7). Source de vérité code : budget de stockage local ([`app-patient/lib/src/doctor/storage_budget.dart`](./app-patient/lib/src/doctor/storage_budget.dart) — `maxQueueEntryBytes` == `PerfBudget.maxCompressedBlobBytes`, `maxPendingQueueEntries`, invariant « aucune image lourde sur l'appareil » #23). Petits correctifs d'accessibilité appliqués (en-têtes sémantiques + libellé TalkBack de l'info vitale « allergie », robustesse à l'échelle de texte) dans [`record_view_screen.dart`](./app-patient/lib/src/ui/record_view_screen.dart). Gate de cohérence doc↔code [`scripts/check-lowend-docs.sh`](./scripts/check-lowend-docs.sh) câblé (`just lowend-check`, dans `just lint`). **Arrivent en phase tests :** garde-fous robustesse host-only (pression disque / atomicité coupure / reprise de file), cas de régression sécurité `LOWEND-*`, invariants d'accessibilité (widget tests cibles ≥ 48 dp / `Semantics` / `textScaleFactor`), fumée PWA + garde-fou de budget de stockage. **Restant (démarche humaine + device-backed) :** la **validation terrain des deux parcours** sur l'appareil Infinix réel (preuve du critère — non closable par du code, mutualisable avec #28/#31) ; durabilité WAL SQLCipher sous vrai kill process, faible RAM réelle, audit Accessibility Scanner — non exécutables en CI host-only.
